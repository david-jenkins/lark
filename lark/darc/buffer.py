...
class Buffer:
  ...
  pb->hdr[0]+=(16-((((unsigned long)pb->arr)+pb->hdr[0])&0xf))%16;
    ...
    def getLabels(self):
        ...
        n=self.getNEntries()
        for i in range(n):
            l.append(self.labels[i].tobytes().decode().strip("\0"))
        return l
    ...
    def getComment(self,name):
        ...
        if self.lcomment[i]>0:
            return self.buffer[self.start[i]+self.nbytes[i]:self.start[i]+self.nbytes[i]+self.lcomment[i]].tobytes().decode()
        else:
            return ""
    ...
    def makeval(self,indx):
        if self.type[indx].decode() in numpy.typecodes["All"]+'c':
            ...
            if self.ndim[indx]>0:
                ...
                val=val[0]
        elif self.type[indx].decode() == 'n':#None...
            val=None
        elif self.type[indx].decode() == 's':#string
            val=self.buffer[self.start[indx]:self.start[indx]+self.nbytes[indx]]
            val = val.tobytes()
            if self.ndim[indx]==0:
                val=val.decode()
...
"""
        ...
        for i in range(len(keys)):
            ...
            txt+=hdr.tobytes()
            txt+=pbuf.buffer[:pbuf.getMem()].tobytes()
        return numpy.frombuffer(txt,dtype='b')
...
class Circular:
    ...
    def __init__(self,shmname,owner=0,dims=None,dtype=None,nstore=None,raw=0,dirname="/dev/shm"):
        ...
        self.condwait=self.buffer[64:64+msize] # drj 140422: changed futex to condwait
