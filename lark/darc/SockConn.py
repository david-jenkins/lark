...
#$Id: b02e3e8f776e6fd28e412361c8a95d453fb0fa80 $
import socket,serialise,types,os,select,pickle,_thread,time
import ConnObj
import sys
import traceback
class dummyLock:
...
class SockConn:
    ...
    def getNoWaiting(self):
        selOut=[]
        for k in list(self.writeDict.keys()):
    ...
    def loop(self):
        ...
        while self.go:
            ...
            for s in rtw:
                ...
                if s in self.writeDict and len(self.writeDict[s])>0:
...
##             rtObjects=None
        ...
        try:
            ...
            if rtObjects==None:#return all objects created...
                rtObjects=list(d.keys())
            elif type(rtObjects) not in [list,tuple]:
                rtObjects=[rtObjects]
            ...
            if l>0:#don't reply if nothing to reply with!
                if sock!=None:
                    ...
                    try:
                        ...
                        rtval=1
                        print("Serialise failed in SockConn.execCmd for tag %s with keys %s (error: %s %s)"%(str(tag),str(list(rt.keys())),str(sys.exc_info()),str(sys.exc_info()[1].args)))
        self.lock.release()
