...
GITID="$Id: 4abfac2d3cc3e9f94a21a232834a54ce85567232 $"
import numpy
import time
import os
#import thread
...
class Control:
    ...
    def __init__(self,globals, options=None):
        ...
        self.sockConn=SockConn.SockConn(self.port,host=self.host,globals=self.globals,startThread=0,listenSTDIN=0,userSelList=list(self.pipeDict.keys())+[self.wakePipeEnd,self.watcher.myfd()],userSelCmd=self.handlePipe,connectFunc=self.newConnection,lock=self.lock)
        ...
        for sock in self.sockConn.selIn:#infoDict["subscribers"].keys():
            ...
            if type(sock)==type(0) or type(sock._sock)==socket._closedsocket:#socket has been closed.
                ...
                if sock not in [self.sockConn.lsock,sys.stdin,self.sockConn.fsock]+self.sockConn.userSelList:#a client
                    if sock in self.sockConn.writeDict:
    ...
    def initialiseRTC(self,startCirc=1):
        ...
        while cnt>0:
            ...
            if self.rtcStarted==0:#RTC not started - start it...
                if canstart:
                    ...
                    for st in list(self.nstoreDict.keys()):#circular buffer sizes
        ...
        if startCirc or corestarted:
            #self.circBufDict={}
            for k in list(self.pipeDict.keys()):
    ...
    def streamCreated(self,name):
        ...
        try:
            if name not in self.streamList:
                ...
                t1=time.time()
                for tag in list(self.paramChangedSubscribers["tag"].keys()):
    ...
    def streamRemoved(self,name):
        ...
        if name in self.streamList:
            ...
            for sock in list(self.pipeDict.keys()):
    ...
    def newConnection(self,s,suberr=1):
        ...
        if suberr:
            ...
            for err in self.errList:
                ...
                if s in self.sockConn.writeDict:
        ...
        if s==self.watcher.myfd():#new stream created, or one deleted.
            ...
            if key==self.shmPrefix+"rtcErrorBuf":
                if infoDict["data"] is None:
                    return False
                err=infoDict["data"].tobytes().decode()
                try:
                    indx=err.index('\0')
                    err=err[:indx]
                except:
            ...
            for sock in list(infoDict["subscribers"].keys()):#self.sockConn.selIn:
                #check that the socket is still open...
    ...
    def circBufThread(self,infoDict,circBufDict,key):
        ...
        while done==0:
            ...
            if firsttime:
                firsttime=0
                msg="%s"%done
                os.write(infoDict["wpipe"],msg[:1].encode())
        ...
        elif key in self.rtcDecimation:
        ...
        for k in list(self.pipeDict.keys()):
            ...
            if infoDict["key"]==key:
                ...
                if freq<=0:#remove ourself from the subscribe list
                    if sock in list(infoDict["subscribers"].keys()):
                        del(infoDict["subscribers"][sock])
                    ...
                    if sock not in list(infoDict["subscribers"].keys()):
                        infoDict["subscribers"][sock]=Subscriber(sock,freq)
                    else:#update the frequency
                        sub=infoDict["subscribers"][sock]
                        sub.freq=freq
                    if key in self.circBufDict:
    ...
    def setRTCDecimation(self,key,freq):
        ...
        if key!=None:
            ...
            if key not in self.circBufDict:
        ...
        if update:
            d={}
            for key in list(self.circBufDict.keys()):
    ...
    def getRTCDecimation(self,key=None):
        ...
        if key==None:
            key=list(self.circBufDict.keys())
        elif type(key)!=type([]):
            key=[key]
        rt={}
        for k in key:
    ...
    def informParamSubscribers(self,pdict=None):
        ...
        changed=list(pdict.keys())
        for s in list(self.paramChangedSubscribers.keys()):
    ...
    def newParamTag(self):
        while self.paramTagRef in self.paramChangedSubscribers["tag"]:
            self.paramTagRef+=1
    ...
    def watchParam(self,tag,paramList=None,timeout=None):
        ...
        """
        if tag not in self.paramChangedSubscribers["tag"]:
            if paramList==None:
    ...
    def revertSavedState(self):
        ...
        self.copyToInactive()
        for key in list(self.savedState.keys()):
    ...
    def getVersion(self):
        version="control.py version: "+GITID+"\ndarccore version:"
        try:
            b=self.getActiveBuffer()
    ...
    def startSummer(self,stream,nsum,decimation=1,affin=0x7fffffff,prio=0,fromHead=1,startWithLatest=1,rolling=0,dtype="n",outputname=None,nstore=10,sumsquare=0):
        ...
        if sumsquare:
            plist.append("-2")
        if outputname in self.summerDict:
    ...
    def startSplitter(self,stream,readfrom=0,readto=-1,readstep=1,readblock=1,affin=0x7fffffff,prio=0,fromHead=1,outputname=None,nstore=-1):
        ...
        if len(self.shmPrefix)>0:
            plist.append("-s%s"%self.shmPrefix)
        if outputname in self.splitterDict:
    ...
    def startBinner(self,stream,nx,ny=1,readfrom=0,readto=-1,stride=-1,dtype='n',decimation=1,affin=0x7fffffff,prio=0,fromHead=1,outputname=None,nstore=-1):
        ...
        if len(self.shmPrefix)>0:
            plist.append("-s%s"%self.shmPrefix)
        if outputname in self.binnerDict:
    ...
    def initialiseBuffer(self,nb,configFile):
        ...
        if type(configFile)==numpy.ndarray:
            ...
            exec(compile(open(configFile, "rb").read(), configFile, 'exec'),d)
            control=d["control"]
            if "comments" in d:
                comments=d["comments"]
        ...
        if bufDone==0:
            ...
            for key in numaList:
                ...
                for name in list(numaData.keys()):
...
if __name__=="__main__":
    ...
    * more information, please use --help
    ...
    if options.cleanStart:
        yn=input("Remove /dev/shm/%srtcParam1 and 2? [y]/n "%options.prefix)
        if yn in ["","y","yes","Y","Yes","YES"]:
            ...
            try:
                ...
                pass
        yn=input("Killall instances of darc (regardless of prefix)? [y]/n ")
        if yn in ["","y","yes","Y","Yes","YES"]:
    ...
    if c.ctrllogread!=None:
        c.ctrllogread.go=0
    darc.unbind(ei=ei, controlName=controlName)
