...
GITID="$Id: 6fef486df64d00a84f407c1a46c8f16f3404c315 $"
import sys
import traceback
import numpy
...
def encodeCorbaArray(val,typ=None):
    if type(val) in [numpy.ndarray,numpy.memmap]:
        if val.dtype==numpy.float32:
            rt=control_idl._0_RTC.Control.FDATA(val.size,val.tobytes())
        elif val.dtype==numpy.int8:
            rt=control_idl._0_RTC.Control.BDATA(val.size,val.tobytes())
        elif val.dtype==numpy.uint16:
            rt=control_idl._0_RTC.Control.UHDATA(val.size,val.tobytes())
    elif type(val)==type([]):
        if typ==[str]:
