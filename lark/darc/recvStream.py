...
import serialise
import os,threading,sys,socket,_thread
import select
import Saver
#import traceback
DataSwitch=None
...
class dc:
    ...
    def process(self,sock):#,readSock=1):
        ...
        try:
            ...
            sock.close()
            if sock in self.sockStreamDict:
        ...
        for data in self.recDataList:
            tag=(data[1])
            if tag in self.dataProcessDict:
    ...
    def dsConfigCallback(self,msg,config):
        ...
        for obj in config.generic:
            if obj.name in self.streamList:
                ...
                if obj.name not in self.saveStream:
    ...
    def sendToDataSwitch(self,name,data):
        ...
        try:
            ...
            if self.serverObject!=None:
                ...
                a=DataSwitch.DataSwitchModule.Generic(1,data.dtype.char, int(fno),float(ftime),len(data.shape),data.shape, data.size*data.itemsize, data.tobytes())
                try:
