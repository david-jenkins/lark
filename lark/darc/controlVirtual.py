...
GITID="$Id: fee2169a6578826265bb00c20101a0d0fe5fe731 $"
import sys
import numpy
import string
import time
...
#        nSkip=timeDelayMirror/2#assume 500Hz = 2ms per iter, so no of iters is timeDelayMirror/2
        try:
            ...
            fdata=self.encode(pmx,numpy.ndarray)#control_idl._0_RTC.Control.FDATA(pmx.size,pmx.tobytes())
        except:
            self.l.release()
            self.raiseErr()
    ...
    def CmakeInteractM(self,data,steps,lock=1):
        ...
        try:
            ...
            if retcorba:#make it into a CORBA object.
                if pmx.dtype!=numpy.float32:
                    pmx=pmx.astype(numpy.float32)
                pmx=self.encode(pmx,numpy.ndarray)#control_idl._0_RTC.Control.FDATA(pmx.size,pmx.tobytes())
        except:
            if lock:
                self.l.release()
            self.raiseErr()
    ...
    def Set(self,names,vals,comments,doSwitch,check,copy):
        ...
        # print("got :",names,vals,comments,doSwitch,check,copy)
        try:
            ...
            for i in range(len(names)):
                ...
                try:
                    # print("trying ",name,val,comment,check)
    ...
    def GetActuators(self,retcorba=1):
        ...
        try:
            ...
            if retcorba:#make it into a CORBA object.
                ...
                acts=self.encode(acts,numpy.ndarray)#control_idl._0_RTC.Control.UHDATA(acts.size,acts.tobytes())
        except:
            self.l.release()
            self.raiseErr()
        self.l.release()
    ...
    def GetStream(self,name,latest,wholeBuffer):#doesn't need lock
        #self.l.acquire()
        #try:
        # print("types",type(name),type(latest),type(wholeBuffer))
...
class Control:
    ...
    def set(self,name,val,com="",swap=1,check=1,copy=1):
        ...
        # print(self.obj)
        # print("setting in ControlVirtual Control")
    ...
    def GetVersion(self):
        ...
        data+="\nlocal controlVirtual.py version:"+GITID
        return data
    def localRead(self,name,callback,lock,done,decimate,sendFromHead,resetDecimate,readFrom,readTo,readStep,latest=0,contiguous=0):
        ...
        print("starting local read thread")
        while go:
