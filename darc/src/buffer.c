...
int bufferCheckNames(int n, char *paramList)
{
  ...
  for (i = 0; i < n; i++)
  {
    if (i > 0 && strncmp(&paramList[(i - 1) * BUFNAMESIZE], &paramList[i * BUFNAMESIZE], BUFNAMESIZE) >= 0)
    { //alphabetical order?
...
int bufferGetIndex(paramBuf *pbuf, int n, char *paramList, int *index, void **values, char *dtype, int *nbytes)
{
  ...
  while (i < nhdr && buf[16 * i] != '\0')
  { //go through everything in the buffer
    //quick find version...
    /*
    s=0;
    e=n;
...
/**
   This function is similar to the above except it also requires and ndim, and dim argument and returns these for each param that's available
   Returns the index of each param named in paramList, or -1 if not found, placed into array index, which should be of size n.  paramList must have n entries, each a null terminated string.
   pbuf is the parameter buffer.
   values is an array of void* with size n.  Each entry is then a pointer to the data, that can be cast as required according to pbuf->nbytes[index] and pbuf->dtype[index], for example, if these are 4 and i, you would do *(int*)(values[index])
   If these are 16 and f, you would do (float*)(values[index])
...
int bufferGetIndexWithShape(paramBuf *pbuf, int n, char *paramList, int *index, void **values, char *dtype, int *nbytes, int *ndim, int (*dim)[6])
{
  int i = 0, j, k;
  char *buf = pbuf->buf;
  int nhdr = BUFNHDR(pbuf);
  int nfound = 0;
  int s;
  //printf("BufferGetIndexWithShape %d %p %p\n",n,buf,paramList);
  wmemset(index, -1, n); //initialise to -1.
  while (i < nhdr && buf[16 * i] != '\0')
  { //go through everything in the buffer
    for (j = 0; j < n; j++)
    { //compare it with our paramList.
      ...
      { //match found
        ...
        dtype[j] = pbuf->dtype[i];
        nbytes[j] = pbuf->nbytes[i];
        ndim[j] = BUFNDIM(pbuf, i);
        for (k=0;k<ndim[j];k++){
          dim[j][k] = BUFDIM(pbuf, i)[k];
        }
        nfound++;
        //printf("Found %16s\n",&paramList[j*BUFNAMESIZE]);
        break;
  ...
  //printf("bufferGetIndexWithShape done, nfound=%d/%d\n",nfound,n);
  return nfound;
}
...
int bufferSetIgnoringLock(paramBuf *pbuf, char *name, bufferVal *value)
{
  ...
  if (indx >= 0 && rt == 0)
  {
    ...
    if (BUFNBYTES(pbuf, indx) + BUFLCOMMENT(pbuf, indx) < value->size)
    { //no space at current location.
    ...
    if (rt == 0)
    {
      ...
      BUFNDIM(pbuf, indx) = value->ndim;
      for (i = 0; i < value->ndim; i++)
      {
        BUFDIM(pbuf, indx)[i] = value->dim[i];
      }
...
bufferVal *bufferGet(paramBuf *pbuf, char *name)
{
  ...
  else if (err == 0)
  {
    ...
    {
      // int itemsize = getDtypeSize(dtype);
      val->size = nbytes;
      val->dtype = dtype;
      val->data = values;
      val->ndim = 0;
...
bufferVal *bufferGetWithShape(paramBuf *pbuf, char *name)
{
  //Note, returned memory should be free'd when finished with.
  int err = 0;
  //int n=0;
  ...
  {
    bufferGetIndexWithShape(pbuf, 1, bufferNames, &indx, &values, &dtype, &nbytes, &ndim, &dim);
  }
  if (indx == -1)
  { //not found
    if (err == 0)
...
bufferVal *callocbufferVal()
{
  bufferVal *bval;
  if ((bval = calloc(1, sizeof(bufferVal))) == NULL)
  {
    printf("Error allocating for bval in mallocBufferVal\n");
    return NULL;
  }
  bval->data = NULL;
  bval->ndim = 0;
...
int bufferCopyToInactive(paramBuf *pbufs[2]){
  int active = bufferGetActive(pbufs);
  memcpy(pbufs[1-active]->buf,pbufs[active]->buf,pbufs[active]->arrsize-((int *)pbufs[active]->arr)[0]);
  return 0;
}