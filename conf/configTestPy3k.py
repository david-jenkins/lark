...
threadAffinity[0] = 1<<1 # control thread affinity
...
subx = [10,10]#npxlx//2#npxl
# suby=(npxly-yoff*2)/nsuby#[10]*ncam#(npxly-8)/nsuby
suby = [10,10]#npxly//2#npxl
for k in range(ncam):
...
try:
    ...
    fname=b"/home/canapyrtc/git/canapy-rtc/ncam2-240x240-264x170-10.fits"
    if not os.path.exists(fname):
        raise(Exception("File does not exist!"))
    while len(fname)%4!=0:#zero pad to it fits into 32bit int array
        fname+=b"\0"
    cameraParams=numpy.frombuffer(fname,dtype="i")
    camerasOpen = 1
...
# camList = ["Pleora Technologies Inc.-iPORT-CL-Ten-Full--OCAM2","EVT-HB-1800SM-640002-ESO"][:ncam]
...
# cameraParams[0:ncam]=16,8#16 bpp - though aravis should be told it is 8 bpp.
# cameraParams[ncam:2*ncam]=65536,65536#block size
# cameraParams[2*ncam:3*ncam]=0,0#x offset
# cameraParams[3*ncam:4*ncam]=0,0#y offset
# cameraParams[4*ncam:5*ncam]=1056,npxlx[1]#npxlx
# cameraParams[5*ncam:6*ncam]=85,npxly[1]#npxly
# cameraParams[6*ncam:7*ncam]=0,0#byteswapInt (1 for ocam)
# cameraParams[7*ncam:8*ncam]=5,0#reorder (2 or 3 for the ocam, 0 for no reordering.  4 is for the array specified below...)
# cameraParams[8*ncam:9*ncam]=50,50#priority
...
control.update({
    ...
    "camerasFraming":1,
    "cameraName":cameraName,#"libcamfile.so",#"camfile",
